...
def write_to_requests(id_user, url, board_name):
    ...
    try:
        ...
        bot.send_message(id_user, "Твій запит додано до бази, тепер ти будеш отримувати нові оголошення першим!\n\n"
                                  "Щоб переглянути та керувати своїми запитами перейди на вкладку \"Мої запити\"", reply_markup=greet_kb)
    except Exception:
        bot.send_message(id_user, "Такий пошуковий запит вже створено, переглянь вкладку \"Мої запити\"", reply_markup=greet_kb)
        ...
        for adv in new_adv:
            ...
            try:
                ...
                if exists('img/' + str(adv[1]) + '.png'):
                    bot.send_photo(user_id, open('img/' + str(adv[1]) + '.png', 'rb'), caption=str_to_send[:1020], reply_markup=greet_kb)
                else:
